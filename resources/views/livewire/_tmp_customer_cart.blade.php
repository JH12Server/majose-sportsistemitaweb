                                    ...
                                    <div class="flex items-center space-x-4 p-4 border border-gray-200 rounded-lg">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if(!empty($item['customization']['file']))
                                                @php $file = $item['customization']['file']; $exists = \Illuminate\Support\Facades\Storage::disk('public')->exists($file); $url = $exists ? \Illuminate\Support\Facades\Storage::disk('public')->url($file) : null; @endphp
                                                @if($exists)
                                                    <div class="mt-2">
                                                        <h5 class="text-sm text-gray-700">Imagen de referencia</h5>
                                                        <img src="{{ $url }}" class="mt-1 h-24 w-24 object-cover rounded cursor-pointer" alt="Referencia" onclick="openPreviewModal('{{ $url }}')">
                                                    </div>
                                                @endif
                                            @endif
                                        </div>
